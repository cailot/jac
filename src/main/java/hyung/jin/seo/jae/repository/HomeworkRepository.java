...
public interface HomeworkRepository extends JpaRepository<Homework, Long>{  
	...
	@Query("SELECT new hyung.jin.seo.jae.dto.HomeworkDTO(h.id, h.type, h.path, h.duration, h.week, h.year, h.info, h.active, h.grade.id, h.subject.id, h.registerDate) FROM Homework h WHERE (?1 = 0 OR h.subject.id = ?1) AND (?2 = 'All' OR h.grade.code = ?2) AND (?3 = 0 OR h.year = ?3) AND (?4 = 0 OR h.week = ?4)")

...
public interface PaymentRepository extends JpaRepository<Payment, Long>{  
	...
	@Query(value = "SELECT p.id, p.amount, p.total, p.method, p.info, p.registerDate, p.invoiceId, p.invoiceHistoryId FROM Payment p WHERE p.invoiceId = ?1 ORDER BY p.id DESC", nativeQuery = true)

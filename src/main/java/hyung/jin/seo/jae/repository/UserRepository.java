...
public interface UserRepository extends JpaRepository<User, String>{  
	...
	@Modifying
	@Query(value = "UPDATE User u SET u.password = ?2 WHERE u.id = ?1 AND u.enabled = 1", nativeQuery = true)
    void updatePassword(Long id, String password);    

    @Query(value = "SELECT u.username, u.password, u.enabled, u.firstName, u.lastName, u.role, u.state, u.branch, u.email, u.phone FROM User u WHERE u.username =?1 AND u.enabled = 1", nativeQuery = true)   
	...
	@Modifying
	@Query(value = "UPDATE User u SET u.password = ?2 WHERE u.username = ?1 AND u.enabled = 1", nativeQuery = true)
